...
"""
- 此模块用于检测视频信号
- 输入为视频源，线程ID，检测位置(房间号)，传入的数据库操作者对象
- 当产生异常视频时，写入数据库异常视频记录
"""
...
def kpt2bbox(kpt, ex = 20):
    ...
    return np.array((kpt[:, 0].min() - ex, kpt[:, 1].min() - ex, kpt[:, 0].max() + ex, kpt[:, 1].max() + ex))


# 主函数，实现实时人体摔倒检测、跟踪、人脸检测和表情识别
# if __name__ == "__main__":

def EF_Detector(source, ID, room_number, DB_Manager,event):
    DB_Manager.Set_Detector_state_on(ID)
    #     source = 0#"import_PoseR/TestData/down1.mp4"  # 使用主函数请把这些参数取消注释掉
    #     ID = 1
    #     room_number = 100
    """
    source 视频源 # detector.source
    ID 检测器线程ID  # detector.threadID
    room_number 检测房间号  # detector.room_number
    anc = annunciations() 报警器
    """
    global abnormal_flag, abnormal, annunciator  # 异常状况标记，异常状况类别，异常状况报警器
    global resize_fn  # 预处理函数
    # =====================================初始化姿态识别==================================================================================================
    # 解析命令行参数
    ...
    par.add_argument('-C', '--camera', default = source, help = 'Source of camera or video file path.')
    ...
    par.add_argument('--save_out', type = str, default = '', help = 'Save display to video file.')
    ...
    detection_model_path = r'E:/Project/OHD_Project/OHD_Project_Detector/import_MotionR/face_classification_master/trained_models/detection_models/haarcascade_frontalface_default.xml'
    # 表情识别模型
    ...
    emotion_model_path = r'E:/Project/OHD_Project/OHD_Project_Detector/import_MotionR/face_classification_master/trained_models/emotion_models/fer2013_mini_XCEPTION.102-0.66.hdf5'
    # 表情识别Label
    ...
    while cam.grabbed():
        ...
        for face_coordinates in faces:
            ...
            draw_bounding_box(face_coordinates, frame, face_color)
            draw_text(face_coordinates, frame, emotion_mode, face_color, 0, -45, 1, 1)
        #  ------------------循环对图像进行人脸表情识别----------------------------------------------------------------------------------------------------
        ...
        # --------------------判定添加异常记录-----------------------------------------------------------------------------------------------------------------
        if abnormal_flag == 0:
            DB_Manager.Set_abnormal_record(abnormal_flag, ID, room_number)
    # --------------------判定异常记录-------------------------------------------------------------------------------------------------------------------------
        if event.is_set():
            break
    # --------------------释放资源-----------------------------------------------------------------------------------------------------------------------------
